...
describe("Put /api/articles/:slug", () => {
  ...
  it("Missing jwt-token", function(doneFn) {
    frisby
      .put('http://127.0.0.1:8080/api/articles/' + slug,
      {
        article: {
  ...
  it("Update article with invalid user", function(doneFn) {
    frisby
      .setup({
  ...
  it("Update article with wrong user", function(doneFn) {
    frisby
      .setup({
      ...
      .expect('status', 403)
      .expect('header', 'Content-Type', 'application/json; charset=utf-8')
      .expect('json', { errors: { article: 'may not be updated' } })
      .done(doneFn);
  });

  it("Update existing article", function(doneFn) {
    frisby
      .setup({
  ...
  it("Update non existing article", function(doneFn) {
    frisby
      .setup({
        request: {
          headers: { 'Authorization': 'Bearer ' + token }
      ...
      .put('http://127.0.0.1:8080/api/articles/' + d.slug_invalid,
      {
        article: {
          title: d.title_new, description: d.description_new,
          body: d.body_new, tagList: d.tags_new
